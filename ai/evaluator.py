...
- 家賃: 手取り30万+補助2万。管理費込み11〜12万がベスト、12.5万以下がスイートスポット、13万超は少し減点、13.5万が絶対上限（超えたらスコア0）
...
【建物の好み】
- モダンなデザイナーズ系外観（コンクリート打ちっぱなし、ダーク/グレー/ベージュ系）を最重視
- ブランド物件に好感（LEGALAND, ASTILE, ウェルスクエア, アバンティア等）
- 3〜5階建ての低層RC/SRCマンション（小規模14戸前後）は *わずかな加点* 要素であり、絶対条件ではない
- **階数について**: 低層へのこだわりは弱い。エレベーターがあれば中高層（6階以上）でも問題なく許容する。
  階数単独で減点しないこと。高層＝自動的にマイナスではない。
...
- 敷礼なし or 少額を好む
- 仲介手数料不要は大きなプラス
- 管理費込み11〜12万がベスト。12.5万以下ならスイートスポット。13万超は少し減点。13.5万が絶対上限（超えたら入居不可＝スコア0）
...
## 【絶対ルール】家賃スコア補正（最重要・他の加点より優先）
管理費込み家賃 (rent + management_fee) に応じて以下の補正を必ず適用:
- **11.0万以下**: +10点ボーナス（ベストゾーン）
- **11.0万〜12.5万**: +5点ボーナス（スイートスポット／理想値、ここに入れば強くおすすめしやすい）
- **12.5万〜13.0万**: ±0点（許容範囲だが理想より上）
- **13.0万〜13.5万**: -10点（上限に近い、減点対象）
- **13.5万超**: スコア0にする（絶対上限超過＝入居不可）
この補正を駅ランクと独立に適用し、最終 SCORE に反映させてください。
